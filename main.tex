...
\item jets fake isolated leptons, photons, or hadronic τ candidates;
...
The signal process is H$\to$WW*$\to$ℓνℓν. The dominant backgrounds considered are non-resonant WW, ttbar, and Z$\to$ττ. The analysis constructs a continuous CGLMP-inspired observable \(I_3\) and performs a template-shape hypothesis test between entangled and separable signal hypotheses.
...
\item Z$\to$ττ normalization uncertainty: 15\%,
\item additional shape uncertainties from cDDPM unfolding closure residuals.
\end{itemize}

This is a useful example of HistFactory/RooFit-style simultaneous template fitting, but it is not a complete data-driven background estimate. A full experimental HWW analysis would also need data-driven control regions for WW, ttbar, Z$\to$ττ, and fake/non-prompt lepton backgrounds. In particular, fake-lepton or multijet backgrounds would usually require fake-factor, matrix-method, or same-sign control-region techniques.
...
\begin{itemize}
\item Standard tools for fake-lepton and fake-τ backgrounds.
...
\textbf{Basic idea}: use same-sign lepton or τ pairs as a fake-enriched control region and extrapolate to opposite-sign signal regions using an OS/SS ratio.

\textbf{Advantages}

\begin{itemize}
\item Simple and data-driven.
\item Useful for dilepton or τ analyses.
...
Pass/fail / tag-rate & Predict tagged region from untagged or fail-tag data & Boosted objects, b-tag, τ-tag, photon fake & Natural for tagger analyses & Tagger-mass correlations, high-dimensional dependence \\ \hline
...
Fake-factor method & Apply tight/anti-tight ratio to fail-ID events & Fake τ, fake lepton, fake photon & Shape can come from data & Anti-ID to ID transfer uncertainty \\ \hline
Same-sign method & Use SS data to predict OS fake background & Dilepton/τ fake estimates & Simple fake-enriched CR & OS/SS ratio systematics \\ \hline
